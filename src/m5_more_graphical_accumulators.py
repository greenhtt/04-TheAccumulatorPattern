...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    for k in range(n):
        ...
        diffx = rectangle1.get_lower_left_corner().x - c1.x
        diffy = rectangle1.get_lower_left_corner().y - c1.y
        c1 = rg.Point(c1.x + diffx * k, c1.y + diffy * k)
        c2 = rg.Point(c2.x + diffx * k, c2.y + diffy * k)
        line = rg.Line(c1, c2)
        line.thickness = 5
        if (k + 1) % 2 == 0:
